...
void Command_Analysis::Receive_LoRa_Cmd(void)
{
  ...
  while ((LoRa_Serial.available() > 0) && (millis() < (Rcv_OverTime + 3000)))
  {
    ...
    /*防止越界*/
    if (gReceiveLength >= 128)  gReceiveLength = 0;
...
{ 
  ...
  if (Verify_Frame_Validity(4, 10, true, false) == true)
  {
    ...
    else
    {
      ...
      Message_Receipt.General_Receipt(AssignGroupIdArrayErr, 2);
    }
  }
  memset(gReceiveCmd, 0x00, gReceiveLength);
}
  ...
  if (Verify_Frame_Validity(4, 15, false, false) == true)
  {
    if (SN.Save_SN_Code(&gReceiveCmd[10]) == true && SN.Save_BKP_SN_Code(&gReceiveCmd[10]) == true)
    {
      ...
      if (Roll_Operation.Save_Area_Number(gReceiveCmd[7]) == true)
      {
        ...
        Message_Receipt.General_Receipt(SetSnAndSlaverCountOk, 2);
      ...
      else
      {
        ...
        Set_Motor_Status(STORE_EXCEPTION);
        Message_Receipt.General_Receipt(SetSnAndSlaverCountErr, 2);
      }
    }
    else
    {
...
void Command_Analysis::ResetRoll_Command(void)
{
  ...
  //FE A0 20 07 C0 01 55 1C 55 01 55 D9 0D 0A0D0A0D0A
  ...
  if (Verify_Frame_Validity(4, 6, true, true) == true)
  {
    ...
    {
      /* 如果当前是正在卷膜，跳转到重置行程 */
      if (gOpeningWorkingFlag)
      {
        Message_Receipt.General_Receipt(RestRollerOk, 2);
        gJumpRunResetRollFlag = true;
        Serial.println("From Opening Jump to ResetRoll... <ResetRoll_Command>");
        return;
      }
      /* 如果当前是重置行程，或者强制卷膜，不能再次进入重置行程了 */
      else if (gResetRollWorkingFlag == true || gForceRollWorkingFlag == true)
      {
...
void Command_Analysis::Opening_Command(void)
{
  ...
  if (Verify_Frame_Validity(4, 7, true, true) == true)  //如果校验通过（CRC校验、区域号校验、组号校验）
  {
    ...
    /* 如果当前正在强制卷膜，如果方向相反，转到另一个方向强制卷膜，如果相同，忽略*/
    if (gForceRollWorkingFlag)
    {
      if (gReceiveCmd[10] == 0xF0 || gReceiveCmd[10] == 0xF1)
      {
        if ((gReceiveCmd[10] == 0xF0 && Roll_Operation.Read_Current_Opening_Value() == 0) ||
            (gReceiveCmd[10] == 0xF1 && Roll_Operation.Read_Current_Opening_Value() == 100))
        {
          Serial.println("The same opening value, not to FroceRoll again <Opening_Command>");
        }
        else
        {
          /* 准备强制相反方向卷膜 */
          gJumpNextOpening = gReceiveCmd[10];
          gJumpRunFroceOpenFlag = true;
          gForceOpenToForceOpenFlag = true; 
        }
      }
      else
      {
        Serial.println("Currently the motor is ForceRoll, and others opening cannot to do !!! <Opening_Command>");
      }
      return;
    }
    /* 如果当前正在重置行程，接收到强制开关棚命令执行切换，开度卷膜，不理会 */
    else if (gResetRollWorkingFlag)
    {
      if (gReceiveCmd[10] == 0xF0 || gReceiveCmd[10] == 0xF1)
      {
        gJumpRunFroceOpenFlag = true;
        gJumpNextOpening = gReceiveCmd[10];
        Serial.println("From ResetRoll Jump to ForceRoll... <Opening_Command>");
      }
      else
      {
        Serial.println("ResetRolling, not to Opening!!!<Opening_Command>");
      }
      return;
    }
    /* 如果当前正在开度卷膜，接收到开度卷膜命令*/
    else if (gOpeningWorkingFlag)
    {
      Message_Receipt.General_Receipt(OpenRollerOk, 2);
      /* 如果新的开度和上一次发送的开度相同，不执行操作 */
      if (gReceiveCmd[10] == Roll_Operation.Read_Current_Opening_Value())
      {
        ...
        /* 如果本次是强制卷膜，转到强制卷膜 */
      else if (gReceiveCmd[10] == 0xF0 || gReceiveCmd[10] == 0xF1)
      {
        gJumpRunFroceOpenFlag = true;
        gJumpNextOpening = gReceiveCmd[10];
        Serial.println("From Opening Jump to ForceRoll... <Opening_Command>");
      }
      /* 和上一次开度不同，跳转到另外的开度 */
      else
      {
        gJumpRunOpeningFlag = true;
        gJumpNextOpening = gReceiveCmd[10]; //获取从服务器接收的开度值
        Serial.println("From the Opening Jump to other Opening... <Opening_Command>");
      }
      return;      
    }
    ...
    Message_Receipt.General_Receipt(OpenRollerOk, 2); //通用回执，告诉服务器接收到了开度卷膜命令
...
{ 
  ...
  if (Verify_Frame_Validity(4, 11, true, true) == true)
  {
    if(Roll_Operation.Save_Roll_Work_Voltage_and_Report_Interval(&gReceiveCmd[10]) == true)
      Message_Receipt.General_Receipt(LimitRollerOk, 2);
    else
    {
      Serial.println("Save working threshold ERROR !");
      Message_Receipt.General_Receipt(LimitRollerErr, 2);
      Set_Motor_Status(STORE_EXCEPTION);
      Message_Receipt.Working_Parameter_Receipt(2);
    }
  }
