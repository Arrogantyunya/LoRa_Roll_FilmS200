...
#define SEND_DATA_DELAY     1 //发送完一帧数据后延时时间（ms）

volatile unsigned char gMotorStatus = MotorFactoryMode;  //每次开机默认状态未初始化
...
void Receipt::Receipt_Random_Wait_Value(unsigned long int *random_value)
{
  ...
  /* 
   *最小延时  RandomSeed * 1ms + RandomSeed * 0.1ms
   *最大延时  RandomSeed * 1000ms + RandomSeed * 500ms
   */
  *random_value = random(RandomSeed * 1000, 1000000) + random(RandomSeed * 100, 500000);
}
...
/*
 ...
 @return  : 无
 */
void Receipt::Working_Parameter_Receipt(unsigned char times)
{  
  ...
  Clear_Server_LoRa_Buffer();
#endif
  // FE E014 1A C001 00 01 01 2D 5985 1F1E 3C00500024000003F114530000000000 B7
  ...
  /*该设备当前电机状态*/
  ReceiptFrame[ReceiptLength++] = gMotorStatus;
  /*该设备电机当前电压值*/
