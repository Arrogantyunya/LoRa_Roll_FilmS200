...
volatile bool A_Flag = false;      //检测是否有手动开棚行为的第一次，因为是双边沿检测，需要在第二次检测时。进行继电器的关闭
volatile bool B_Flag = false;    //检测是否有手动关棚行为的第一次，因为是双边沿检测，需要在第二次检测时。进行继电器的关闭
...
volatile bool gJumpRunResetRollFlag = false;
volatile bool gJumpRunOpeningFlag = false;
volatile bool gJumpRunFroceOpenFlag = false;
volatile bool gForceOpenToForceOpenFlag = false;  //从强制卷膜跳转到另一个方向的强制卷膜标志位，用来防止强制停止后开度被清除！
volatile unsigned char gJumpNextOpening = 0;
...
void Motor_Operations::Motor_GPIO_Config(void)
{
  ...
  pinMode(OTHER_MOTOR_PIN, OUTPUT);
  OTHER_MOTOR_DIS;
...
bool Motor_Operations::Detect_Motor_Limit(unsigned char *current_opening, Limit_Detection dec, Roll_Action act, unsigned char roll_opening, unsigned int real_roll_time)
{
  ...
  if (gRollingTime < DETECT_START_ROLL_TIME)
    return false;
  ...
  if (Current_Detection() <= 150)
  {
    ...
    if (gRollingTime >= (gLowCurrentTime + 3)) //连续检测到电流小于100mA 3秒
    {
      ...
      /*卷膜方向是开棚的情况下*/
      case Open:
        Serial.println("Reach Up limit... <Detect_Motor_Limit>");
      ...
      /*卷膜方向是关棚的情况下*/
      case Close:
        Serial.println("Reach Down limit... <Detect_Motor_Limit>");
...
{
  if (status == Current_Uninit)
    return Current_Uninit;
  if (status == Current_Exception)
    return Current_Exception;
...
void Motor_Operations::Detect_Manual_Rolling(void)
{
  ...
  else  //如果已经发生按键异常，当检测按键松开后清除按键异常
  {
    if (digitalRead(DEC_MANUAL_UP_PIN) == LOW && digitalRead(DEC_MANUAL_DOWN_PIN) == LOW)
      {
        if (Current_Detection() < 100)
          gManualKeyExceptionFlag = false;
      }
  }
}
...
void Motor_Operations::Adjust_deviation(void)
{
  ...
  Serial.print("Current_Opening <Adjust_deviation>: ");
  Serial.println(Current_Opening_Temp);

  unsigned char RealTime_Opeing_Temp = Roll_Operation.Read_RealTime_Opening_Value();
  Serial.print("RealTime_Opening <Adjust_deviation>: ");
  Serial.println(RealTime_Opeing_Temp);
...
void Motor_Operations::Jump_Motor_Action_Handler(void)
{
  /* 如果当前想将开度卷膜切换到重置行程 */
  if (gJumpRunResetRollFlag)
  {
    gJumpRunResetRollFlag = false;
    MANUAL_ROLL_OFF;
    detachInterrupt(DEC_MANUAL_DOWN_PIN);
    detachInterrupt(DEC_MANUAL_UP_PIN);
    iwdg_feed();
  ...
  else if (gJumpRunFroceOpenFlag)
  {
    gJumpRunFroceOpenFlag = 0;
    if (gJumpNextOpening == 0xF0 || gJumpNextOpening == 0xF1)
    {
      MANUAL_ROLL_OFF;
      detachInterrupt(DEC_MANUAL_DOWN_PIN);
      detachInterrupt(DEC_MANUAL_UP_PIN);
      ...
      Motor_Operation.Force_Open_or_Close(gJumpNextOpening);
      /*自动卷膜完成后，使能手动卷膜，打开检测手动卷膜按键中断*/
      attachInterrupt(DEC_MANUAL_UP_PIN, Manual_Up_Change_Interrupt, CHANGE);
      attachInterrupt(DEC_MANUAL_DOWN_PIN, Manual_Down_Change_Interrupt, CHANGE);
      MANUAL_ROLL_ON;
  ...
  /* 如果当前想将开度卷膜切换到另一个开度卷膜 */
  else if (gJumpRunOpeningFlag)
  {
    gJumpRunOpeningFlag = false;
    detachInterrupt(DEC_MANUAL_DOWN_PIN);
    detachInterrupt(DEC_MANUAL_UP_PIN);
    MANUAL_ROLL_OFF;
    ...
    {
      if(Roll_Operation.Save_Current_Opening_Value(gJumpNextOpening))  //保存当前开度值
      {
        Serial.println("Begin to coiling... <Break_Motor_Action_Handler>");
        Motor_Operation.Motor_Coiling();  //开始卷膜
        iwdg_feed();
      }
      else  //保存开度值异常
      {
        Serial.println("Save current opening value ERROR !!! <Break_Motor_Action_Handler>");
        Set_Motor_Status(STORE_EXCEPTION);
        Message_Receipt.Working_Parameter_Receipt(2);
      }
    }
    ...
    {
      if (gJumpNextOpening == 0 || gJumpNextOpening == 100)
      {
        if (gJumpNextOpening == 0)
          gJumpNextOpening = 0xF0;
        else
          gJumpNextOpening = 0xF1;

        Serial.println("Prepare Force Open or Close. Be careful... <Break_Motor_Action_Handler>");
        Motor_Operation.Force_Open_or_Close(gJumpNextOpening);
      }
      else
      {
        Serial.println("The film has not measured the distance, first measure, then roll the film... <Break_Motor_Action_Handler>");
        if(Motor_Operation.Reset_Motor_Route() == true) //先重置行程，再开度卷膜
        {
          Serial.println("Roll OK, motor begin coiling... <Break_Motor_Action_Handler>");

          if(Roll_Operation.Save_Current_Opening_Value(gJumpNextOpening))  //保存当前开度值
          {
            gAdjustOpeningFlag = false;
            iwdg_feed();
            MyDelayMs(REVERSING_DELAY);
            iwdg_feed();
            Motor_Operation.Motor_Coiling();  //开始卷膜
          }
          else  //保存开度值操作异常
          ...
          Serial.println("Reset motor route failed !!! <Break_Motor_Action_Handler>");
        iwdg_feed();        
      }
    }
    /*自动卷膜完成后，使能手动卷膜，打开检测手动卷膜按键中断*/
...
{
  if (gJumpRunResetRollFlag || gJumpRunOpeningFlag || gJumpRunFroceOpenFlag) 
    gStopWorkFlag = true;
  ...
  if (gStopWorkFlag == true)
  {
    ...
    else if (act == Opening_Roll)
    {
      ...
      Roll_Operation.Save_Current_Opening_Value(realtime_opening);
      Roll_Operation.Save_RealTime_Opening_Value(realtime_opening);
    }
    /*
    ...
    */
    else if (act == Force_Open || act == Force_Close)
    {
      ...
      if (gForceOpenToForceOpenFlag == false) //如果不是从一个强制卷膜跳转到另一个，被强制停止了，需要清除重置完成标志位
      {
        if (!Roll_Operation.Clear_Route_Save_Flag())
        {
      ...
      else if (gForceOpenToForceOpenFlag == true)
      {
        Serial.println("Prepare from forceroll jump to forceroll... <Force_Stop_Work>");
        gForceOpenToForceOpenFlag = false;
      }
...
bool Motor_Operations::Verify_Reset_OK(unsigned char *current_opening, bool first_A_direction_flag, bool is_first_stage)
{
  ...
  do
  {
    ...
    if (is_first_stage)
    {
      if (first_A_direction_flag)
      {
        if (Detect_Motor_Limit(current_opening, Open, Reset_Roll, 0, 0))
          break;
      ...
      {
        if (Detect_Motor_Limit(current_opening, Close, Reset_Roll, 0, 0))
          break;
      ...
      if (Current_Detection() < 100)
      {
        ...
        if (gVerifyTraceOpeningNum > 10)
        {
          ...
          if (gIsTraceUpOpeningFlag)
          {
            ...
            if (ManualOpening > 100)
              ManualOpening = 100;
            Serial.print("Up roll, manual opening is ");
            Serial.println(ManualOpening);
          }
          else
          {
            ...
            if (ManualOpening > 100)
              ManualOpening = 0;
            Serial.print("Down roll, manual opening is ");
            Serial.println(ManualOpening);
          }
          ...
          Message_Receipt.Working_Parameter_Receipt(1);
        }
...
bool Motor_Operations::Force_Open_or_Close(unsigned char opening_value)
{
  ...
  if (opening_value == 0xF0)
  {
    ...
    CloseFlag = true;
    Roll_Operation.Save_Current_Opening_Value(0);
  ...
  else if (opening_value == 0xF1)
  {
    ...
    OpenFlag = true;
    Roll_Operation.Save_Current_Opening_Value(100);
  ...
  do
  {
    ...
    if (OpenFlag)
    {
      if (Detect_Motor_Limit(&RecentOpening, Open, Reset_Roll, 0, 0))
        break;
      if (Force_Stop_Work(Force_Open) == true)
        return true;
      if (Motor_Current_Status(CurrentThreshold, SavedCurrent, CurrentStatus) == true)
        return false;
    }
    else if (CloseFlag)
    {
      if (Detect_Motor_Limit(&RecentOpening, Close, Reset_Roll, 0, 0))
        break;
      if (Force_Stop_Work(Force_Close) == true)
        return true;
      if (Motor_Current_Status(CurrentThreshold, SavedCurrent, CurrentStatus) == true)
        return false;
    }
  ...
  */
  ...
  do
  {
    ...
    if (ResetFirstDIRFlag)
    {
      ...
      if (Detect_Motor_Overtime(Close))
        return false;
      if (Motor_Current_Status(CurrentThreshold, SavedCurrent, CurrentStatus))
        return false;
    }
    else
    {
  ...
  do
  {
    ...
    else
    {
      if (Detect_Motor_Limit(&RecentOpening, Close, Reset_Roll, 0, 0))
        break;
      if (Detect_Motor_Overtime(Close) == true)
        return false;
      if (Motor_Current_Status(CurrentThreshold, SavedCurrent, CurrentStatus) == true)
        return false;
    }
  ...
  if (Verify_Reset_OK(&RecentOpening, IsFirst_A_Direction, false))
  {
    ...
    Serial.print("Reset rolling time <Reset_Motor_Route> = ");
    Serial.println(SavedRollingTime);
  }
  else
  {
    Serial.println("Verify reset failed!!!");
...
bool Motor_Operations::Motor_Coiling(void)
{
  ...
  if (Roll_Operation.Read_Route_Save_Flag() == true)
  {
    ...
    Serial.print("Last_opening <Motor_Coiling>:");
    Serial.println(LastOpening);
    Serial.print("Recent_opening <Motor_Coiling>:");
    Serial.println(RecentOpening);
    Serial.print("Total_Roll_Time <Motor_Coiling>:");
    Serial.println(TotalOpeningTime);

    iwdg_feed();

    /*边界判断，如果这三个参数的值大于边界范围，视为储存异常*/
    if (LastOpening > 100 || RecentOpening > 100 || TotalOpeningTime > MAX_OPENING_TIME)
    {
    ...
    Serial.print("Voltage original need time <Motor_Coiling> = ");
    Serial.println(RollTimeTemp);
    iwdg_feed();
    ...
    do
    {
      ...
      if (OpenFlag)
      {
        if (Detect_Motor_Limit(&RecentOpening, Open, Opening_Roll, RollOpening, RealRollTime))
          break;
        if (Detect_Motor_Overtime(Open))
          return false;

        if (Force_Stop_Work(Opening_Roll, OpeningTemp) == true)
          return true;
        if (Motor_Current_Status(Current_Threshold, Saved_Current, Current_Status) == true)
          return false;
      }
      else if (CloseFlag)
      {
        if (Detect_Motor_Limit(&RecentOpening, Close, Opening_Roll, RollOpening, RealRollTime))
          break;
        if (Detect_Motor_Overtime(Close))
          return false;
        if (Force_Stop_Work(Opening_Roll, OpeningTemp) == true)
    ...
    if (OpenFlag)
    {
      Serial.print("Opening rolling time <Motor_Coiling>= ");
      Serial.println(gRollingTime);
      Serial.println("# Open shed finishied...<Motor_Coiling>");
    }
    else if (CloseFlag)
    {
      Serial.print("Close rolling time <Motor_Coiling>= ");
      Serial.println(gRollingTime);
...
void Motor_Operations::Calculate_Voltage(unsigned int *voltage_collect_num, unsigned long *voltage_value, int *voltage_value_temp, unsigned int *voltage_calib)
{
  if ((gRollingTime % CURRENT_COLLECTION_FREQ == 0) && (gRollingTimeVarFlag == true))
  {
    ...
    *voltage_collect_num += 1;
    if (*voltage_value_temp < 0)
      *voltage_value_temp *= -1;
...
bool Motor_Operations::Calculate_and_Save_Current(unsigned int *current_collect_num, unsigned int *current_value, unsigned int *current_value_temp, unsigned int *current_calib, Limit_Detection act)
{
  ...
  {
  case Open:
    UpBoolValue = Roll_Operation.Save_Roll_Up_Current(*current_value);
    ...
    {
      Serial.print("Roll up current value(mA) : ");
      Serial.println(*current_value);
      float Current_Coefficient = Roll_Operation.Read_Roll_High_Current_Limit_Value() / 10.0;
      Serial.print("Current_Coefficient = ");
      Serial.println(Current_Coefficient);
      Serial.println(*current_value * Current_Coefficient);
    }
    break;

  case Close:
    DownBoolValue = Roll_Operation.Save_Roll_Down_Current(*current_value);
    ...
    {
      Serial.print("Roll down current value(mA) : ");
      Serial.println(*current_value);
      float Current_Coefficient = Roll_Operation.Read_Roll_High_Current_Limit_Value() / 10.0;
      Serial.print("Current_Coefficient = ");
      Serial.println(Current_Coefficient);
...
void Manual_Up_Change_Interrupt(void)
{
  ...
  if (digitalRead(DEC_MANUAL_UP_PIN) == HIGH)
  {
    ...
    if (digitalRead(DEC_MANUAL_UP_PIN) == HIGH)
    {
      ...
      OTHER_MOTOR_EN;
      Serial.println("OTHER_MOTOR_EN <Manual_Up_Change_Interrupt>");
    }
  }
  else if (digitalRead(DEC_MANUAL_UP_PIN) == LOW)
  {
    ...
    if (digitalRead(DEC_MANUAL_UP_PIN) == LOW)
    {
      ...
      Stop_Roll_Timing();
      OTHER_MOTOR_DIS;
      Serial.println("OTHER_MOTOR_DIS <Manual_Up_Change_Interrupt>");
    }
  } 
  attachInterrupt(DEC_MANUAL_UP_PIN, Manual_Up_Change_Interrupt, CHANGE);
}
...
void Manual_Down_Change_Interrupt(void)
{
  ...
  if (digitalRead(DEC_MANUAL_DOWN_PIN) == HIGH)
  {
    ...
    if (digitalRead(DEC_MANUAL_DOWN_PIN) == HIGH)
    {
      ...
      OTHER_MOTOR_EN;
      Serial.println("OTHER_MOTOR_EN <Manual_Down_Change_Interrupt>");
    }
  }
  else if (digitalRead(DEC_MANUAL_DOWN_PIN) == LOW)
  {
    ...
    if (digitalRead(DEC_MANUAL_DOWN_PIN) == LOW)
    {
      ...
      Stop_Roll_Timing();
      OTHER_MOTOR_DIS;
      Serial.println("OTHER_MOTOR_DIS <Manual_Down_Change_Interrupt>");
    }
  }
  attachInterrupt(DEC_MANUAL_DOWN_PIN, Manual_Down_Change_Interrupt, CHANGE);
}
