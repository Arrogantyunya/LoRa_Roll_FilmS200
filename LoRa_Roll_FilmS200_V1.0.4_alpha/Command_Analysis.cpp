...
void Command_Analysis::Receive_LoRa_Cmd(void)
{
  ...
  while ((LoRa_Serial.available() > 0) && (millis() < (Rcv_OverTime + 5000)))
  {
    iwdg_feed();
...
void Command_Analysis::Query_Current_Work_Param(void)
{
  ...
  if (Verify_Frame_Validity(4, 23, true, false) == true)
  {
    if (gReceiveCmd[8] == 0x01) //配置参数标志（LoRa大棚传感器是0x00配置采集时间）
    {
      ...
      /* 预留位的后7位+新增1位（8位），前4位为发送频点，后4位为接收频点，若是全为0则表示不更改 */
      if (!(gReceiveCmd[20] == 0x00 && gReceiveCmd[21] == 0x00 && gReceiveCmd[22] == 0x00 && gReceiveCmd[23] == 0x00))
      {
        String TFREQ = String(gReceiveCmd[20],HEX)+String(gReceiveCmd[21],HEX)+String(gReceiveCmd[22],HEX)+String(gReceiveCmd[23],HEX);
